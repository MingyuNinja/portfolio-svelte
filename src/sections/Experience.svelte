...
<main class="mx-6 text-left">
    <div id={id} class="my-4 mx-auto max-w-3xl text-light-slate">
        ...
        <article class="my-4 flex items-start gap-4 {hoveredExperience !== null && experience !== hoveredExperience ? 'fade' : ''}"
            on:mouseenter={() => hoveredExperience = experience}
            on:mouseleave={() => hoveredExperience = null}
            on:click={() => window.open(experience.url, '_blank')}
        >
            <div class="bg-transparent py-4 rounded text-slate-200 text-sm w-24 flex-shrink-0">
                {experience.duration}
            </div>
            <div>
